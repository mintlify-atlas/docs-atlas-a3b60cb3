...
The dashboard serves as a simple authenticated page that demonstrates the authentication system is working correctly. It's protected by auth middleware, so only signed-in users can access it.

```typescript src/app/dashboard.tsx
...
From the dashboard, you can sign out using the "Sign Out" button, which clears your session and redirects you to the homepage.
